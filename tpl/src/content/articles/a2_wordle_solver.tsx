...
import model_score_distribution from "@/content/articles/a2_wordle_solver/model_score_distribution.png"
...
export default function Article() {
    return (
        <Body>
            ...
            <Text
                lead="Wordle is a viral daily word puzzle where the player has six attempts to find a hidden five letter word. After each guess, the game tells the user some information about their guess: If each letter is either correct, in the word but not in the right place, or not in the word at all."
                content="Despite being a simple game that almost anyone can play, Wordle is an interesting problem to investigate from a data and algorithmic perspective. Each guess provides information that constrains the remaining solution space, making the game a small but concrete example of inference under uncertainty."
            />
            ...
            <Text
                content="Wordle draws its solutions from a fixed list of valid words, which was originally accessible in the JavaScript source of the game. For this investigation, I restrict candidate solutions to that same list. While this simplifies the solver, it also allows performance to be measured precisely over the same solution space used by the game itself."
            />
...
The goal of the next guess is therefore not necessarily to be correct, but to extract as much information as possible.`}
                bullets={[
                    "Step 1: Maintain a set of candidate words that are still consistent with all feedback seen so far.",
                    "Step 2: Count how often each letter appears across those candidates.",
                    "Step 3: Score each possible guess based on those letter frequencies.",
                    "Step 4: Choose the highest scoring word as the next guess."
                ]}
            />

            <Text
                content={`Step 1: After each guess, reduce the candidate set by removing any words that are inconsistent with the observed feedback.`}
                code={`candidates ← all valid words

for each guess:
    feedback ← compare(guess, answer)
    candidates ← words consistent with feedback`}
            />

            <Text
                content={`Step 2: Build a frequency table by scanning the remaining candidates.
Each letter is only counted once per word, so repeated letters do not artificially inflate a word’s score.`}
                code={`frequency_table ← empty map
...
for each word in candidates:
    for each unique letter in word:
        frequency_table[letter] += 1`}
            ...
            <Text
                content={`Step 3: Assign a score to each possible guess by summing the frequencies of its unique letters.
Words that cover common letters across the candidate set receive higher scores.`}
                code={`score(word) =
    sum of frequency_table[letter]
    for each unique letter in word`}
            />

            <Text
                content={`Step 4: Select the word with the highest score.
If multiple words tie, choose one deterministically to keep results reproducible.`}
                code={`best_guess ← word with maximum score
play best_guess`}
            />


            <Text
                content={`Step 3: Score a word by adding up the frequencies of its unique letters. Words that contain common letters will score higher.`}
                code={`score = sum(freq[ch] for ch in set(word))`}
            />

            <Text
                content={`Step 4: Pick the highest scoring candidate word. Using sorted(...) makes the selection deterministic when multiple words tie for the same score.`}
                code={`best_guess = max(sorted(candidates), key=score)`}
            />


            <Text
                content="This approach performs surprisingly well. Across the full set of valid Wordle answers,
                 the frequency based solver achieves a mean score of 3.482 guesses, which is quite strong performance considering it's ease of implementation. But there's a catch!"
            />
...
A greedy guess can look informative (lots of greens/yellows), while still failing to reduce the candidate space meaningfully.`}
            ...
            <Text
                content={`To see why, consider an endgame situation where earlier guesses have already narrowed the solution down.
Suppose the remaining candidates mostly share the same suffix:`}
                code={`Remaining candidates (example):
WATCH
CATCH
HATCH
MATCH
...
You’ve learned a lot about the form of the word, but you haven’t reduced the remaining options as much as you might expect.`}
            ...
            <Text
                content={`Now consider a different guess. Even though WALTZ contains less “popular” letters (including Z), it tests a more discriminating idea: whether the word starts with W.`}
                code={`Alternative guess:
WALTZ`}
            />

            <Text
                content={`If the W is confirmed, WATCH becomes much more likely. If the W is rejected, then a large chunk of candidates are eliminated immediately.
The key point is that this guess can reduce the candidate set more aggressively, even if the feedback pattern looks less “close” to the final answer.`}
                code={`Interpretation (informal):
If W is confirmed  → WATCH-type words survive
If W is rejected   → WATCH is eliminated, and the set collapses`}
            />

            <Text
                content={`This is the weakness of greedy scoring: it optimises for immediate overlap and common letters, not for how well a guess splits the remaining candidate space.
So how can we better select guesses based on future game states?`}
            />
            ...
            <Text
                content={`At the core of the dynamic programming approach is a recursive evaluation of future game states.
                Each state is defined by the remaining candidate words, and the cost of a guess is the expected number of guesses required to finish the puzzle from that state.`}
                code=
                    {
                    `expected = 1.0
    for subset in partitions:
    probability = len(subset) / len(candidates)
    expected += probability * dp_value(subset)`
            }
            />
            ...
            <Text
                ...
                code={`if len(candidates) <= DP_LIMIT:
    _, guess = dp_value(candidates)
        else:
    guess = select_guess_frequency(candidates)`}
            ...
            <Text
                lead="But what should that threshold be?"
                content="I made a separate script to iterate through a range of limit values , and with this I was able to determine that performance doesn't increase beyond a DP limit of 121, for my data set."
            />
            ...
            <Text
                ...
                The dynamic programming model shifts probability mass toward earlier solves, scoring more 3s, and less 4s and 5s.`}
            />
