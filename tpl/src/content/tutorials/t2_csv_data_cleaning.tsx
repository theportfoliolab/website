...
print(df.info())`}
            ...
            <Text
                content={`You should always start by checking:
...
.set_index("Date") makes Date the DataFrame’s index, making time-based operations easier later.`}
            ...
            <Text
                content={`This shows the number of missing values per column.
You can also view which rows are affected:`}
                code={`print(df[df.isnull().any(axis=1)])`}
            />

            <Text
                content={`Financial data often has missing prices or volumes (e.g. holidays or partial trading days).
...
Dropping rows:`}
            ...
            <Text
                content={`Removes any rows that contain missing values.
Use this only if the dataset is large and the missing portion is small.

Filling values:`}
                code={`df["Close"] = df["Close"].fillna(method="ffill")  # forward fill
df["Volume"] = df["Volume"].fillna(0)             # replace with zero`}
            />

            <Text
                content={`Forward-fill (method="ffill") is common for financial time series — it carries forward the last valid value when data is missing.`}
            />
...
df["Close"] = pd.to_numeric(df["Close"], errors="coerce")`}
            ...
            <Text
                content={`This will convert text-like numbers into floats, replacing non-numeric entries with NaN.
...
print("After:", len(df))`}
            ...
            <Text
                content={`If you suspect duplicates only by date, you can specify:`}
                code={`df = df[~df.index.duplicated(keep="first")]`}
            />

            <Text
                content={`This ensures only one record per date remains.`}
            />
    ...
    "Trans": "Transactions"
            ...
            <Text
                content={`To reorder columns:`}
                code={`df = df[["Code", "Open", "High", "Low", "Close", "Volume"]]`}
            />

            <Text
                content={`A well-structured dataset is easier to read, document, and reuse.`}
            />
...
One quick way to check for them is using .describe():`}
            ...
            <Text
                content={`To filter unrealistic values, you could cap or remove them:`}
                code={`df = df[(df["Close"] > 0) & (df["Close"] < 10000)]`}
            />

            <Text
                content={`This ensures all prices are within a reasonable range.`}
            />
            ...
            <Text
                content={`This fills any missing calendar days with the last known value — useful for time-series models that require continuous data.`}
            />
