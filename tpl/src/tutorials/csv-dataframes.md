# Introduction to CSV Dataframes

#### Financial analysis using Python usually involves some kind of series and running some kind of computation on that series.

### Comma Seperated Values (CSV)
When approaching the task, you are likely to encounter **Comma Separated Value (.csv) files**. These files can hold value for tables.
But alone, a table is not much help to us. We need some kind of way to look through the table, process the data, and perform computations on its values.

### Dataframes
This is where **Dataframes** come in—a Dataframe is a computer object that, once loaded, stores all the data of the .csv file in a much more useful way. This is the starting point for your technical analysis in Python.

---

## Setup
#### To start, I will assume that:
- [x] You are familiar with basic object-oriented programming coding concepts
- [x] You are familiar with Git, that is, you can fetch a repository
- [x] You are familiar with Python development, that is, you can create and run a script using the command line
- [x] You will also need to have the Pandas library installed `$ pip install pandas`
...
Each line in the CSV file represents a "row" of data, where values are separated with a comma in between each value. For example:
```
05898, AWR584, 6/12/1978
02785, BLH967, 4/5/2001
```

Here we see two entries, with three values associated with each entry. Each entry (line in the file) should have the same number of values (rows, or number of commas)

You might recognise the last value in this series as a calendar date. But what if you don't know what the other two columns represent?
...
Now we can see that the first value in each line is that entries `Customer ID`, the second is their `Vehicle rego`, and the last entry is `Date of Birth`. You may sometimes find CSVs which do not contain this line and start right away with data.

To make a well-formed CSV to store this data, put your line of labels at the top, and your data below. Make sure there are no empty lines, or some programs may have trouble working with your .csv file.
...
_Give instructions on fetching the sample code and price data from a repository_

1. Open example.py or create a new python file with the usual script boilerplate:
Check your target CSV is in the same directory as your script.

2. Opening the CSV and creating the dataframe can be done in two lines of code:
...
_Create a graphic explaining each part of the line of code: name for the dataframe, method call, string of filename
Mention in a few words that the first line is better practice for expanding the script for multiple files_
...
```
This saves the Dataframe as a CSV file. Setting `index=False` prevents pandas from writing the row index as an extra column in the output file.
...
This saves the DataFrame to an Excel file. You can specify a sheet name using `sheet_name="Sheet1"`.
...
This writes only the specified columns to the new file.
...
This appends new rows to an existing CSV file instead of overwriting it.
